export default {
  ...
  speakers_dir: {
    ...
    for_backloadedhorn: {
      ...
      abst: "バックロードホーンエンクロージャ向けのスピーカーユニットを選定する基準として、基本的によく言われているのは磁気回路として強力なマグネット搭載し、Qts(Total Quality Factor、Q0とも表記される)値が低くオーバーダンピング傾向にあり、内部欠損が適切かつ剛性の高い素材(多くはペーパーやケナフ、ケブラーなど)を使った部品で構成され、結果としてそもそも能率(Sound Pressure Level、SPL)が高いユニットが選ばれる傾向にあります。ここでは、そもそもメーカーからバックロードホーン向けとして設計されたスピーカーユニットのうち、Qts順に並べました。選定の参考にしてください。",
    }
  },
